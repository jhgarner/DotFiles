...
prompt_pure_preprompt_render() {
	...
	postprompt_parts=('${prompt_pure_state[username]}')
	# Execution time.
	...
	local TERMWIDTH
    (( TERMWIDTH = ${COLUMNS} - 1 ))
  local main_prompt=${(j. .)preprompt_parts}
  local zero='%([BSUbfksu]|([FK]|){*})'
	local prompt_size=${#${(S%%)main_prompt//$~zero/}}
  local pwdsize=${${(S%%)postprompt_parts//$~zero/}}
  PR_FILLBAR="\${(l.(($TERMWIDTH - ($prompt_size + ${#pwdsize} - 14))).. .)}"
